/*
# Notorious.Y2 — Full Supabase Schema v2

Creates the complete database for the Notorious.Y2 storefront + admin panel.

## New Tables
- profiles: user accounts linked to auth.users, with admin flags
- admin_email_allowlist: emails allowed admin access
- addresses: shipping addresses per user
- currencies: exchange rates for multi-currency display
- products: catalog with position/rotation/scale for floor layout
- product_inventory: per-size stock
- inventory_history: stock change log
...
- credit_accounts / credit_transactions: store credit
- discounts / discount_redemptions: coupon codes
- campaigns: email marketing campaigns
- abandoned_carts: cart recovery tracking
- admin_activity_log: admin action log
- admin_notifications: admin notification feed
- banners: storefront banners
- store_settings: key-value store settings
- analytics_events: traffic tracking

## Views
- admin_products_view: products with conversion_rate
- admin_customers_view: profiles with order stats
- admin_revenue_daily: daily revenue aggregation
- admin_kpis: headline KPIs

## Triggers
- handle_new_user: auto-creates profile on signup
- notify_new_customer: admin notification on new customer
- recalc_product_stock: updates product.stock from inventory
- notify_low_stock: admin notification when stock < 5
- bump_product_carts_count: increments carts_count on add-to-cart
- bump_product_sales: increments sales_count on order
- set_order_number: auto-generates NY2-XXXXX order numbers
- notify_new_order: admin notification on new order
- log_order_status_change: activity log on status change
- set_invoice_number: auto-generates invoice numbers
- apply_credit_transaction: updates credit balance
- bump_discount_usage: increments discount used_count

## Security
- RLS enabled on every table
- Public read: products, inventory, currencies, banners, store_settings, discounts (active only)
- Owner-scoped: profiles, addresses, carts, cart_items, wishlist, orders, returns, payment_methods, transactions, credit
- Admin-only: campaigns, abandoned_carts, activity_log, notifications, analytics, admin allowlist
- Storage buckets: product-images, banners (public read, admin write)

## Seeds
- 3 admin emails in allowlist
- 7 currencies
- 4 discount codes
- 7 store settings entries
- 38 products with floor positions
- Per-size inventory (SMALL/MEDIUM/LARGE, 10 each)
*/
...
do $$ begin create type admin_role_type as enum ('Owner','Admin','Manager','Support','Analyst'); exception when duplicate_object then null; end $$;
...
declare
  v_is_admin boolean;
begin
  select exists(select 1 from public.admin_email_allowlist a where lower(a.email) = lower(new.email))
    into v_is_admin;
  ...
  values (
    ...
    v_is_admin,
    case when v_is_admin then 'Admin'::admin_role_type else null end,
    'Active'
  )
...
begin
  ...
  update public.products
    set stock = v_total, sold_out = (v_total = 0)
...
insert into public.discounts (code, type, value, min_order, usage_limit, used_count, starts_at, ends_at, status)
values
  ('Y2K10',    'Percentage',     10,  0,   100, 42,  '2025-08-01', '2025-08-31', 'Active'),
  ('SUMMER15', 'Percentage',     15,  100, 200, 200, '2025-07-01', '2025-07-31', 'Expired'),
  ('FREESHIP', 'Free Shipping',  0,   50,  500, 87,  '2025-08-15', '2025-09-15', 'Active'),
  ('FLAT100',  'Fixed',          100, 300, 50,  0,   '2025-09-01', '2025-09-30', 'Scheduled')
...
insert into public.store_settings (key, value) values
  ...
  ('hero_section', '{"enabled":true,"eyebrow":"NOTORIOUS.Y2","headline":"Built for the Y2 generation.","description":"Contemporary streetwear with a nostalgic Y2K attitude.","button_text":"Shop Collection","button_link":"/"}'),
...
create policy "inventory_admin_write" on public.product_inventory for all using (public.is_admin()) with check (public.is_admin());
...
create policy "returns_owner_or_admin" on public.returns for all using (user_id = auth.uid() or public.is_admin()) with check (user_id = auth.uid() or public.is_admin());

drop policy if exists "payment_methods_owner_or_admin" on public.payment_methods;
create policy "payment_methods_owner_or_admin" on public.payment_methods for all using (user_id = auth.uid() or public.is_admin()) with check (user_id = auth.uid() or public.is_admin());
...
create policy "transactions_admin_write" on public.payment_transactions for all using (public.is_admin()) with check (public.is_admin());
...
create policy "credit_tx_owner_or_admin" on public.credit_transactions for all using (user_id = auth.uid() or public.is_admin()) with check (user_id = auth.uid() or public.is_admin());
...
create policy "campaigns_admin_only" on public.campaigns for all using (public.is_admin()) with check (public.is_admin());
...
create policy "admin_allowlist_admin_only" on public.admin_email_allowlist for all using (public.is_admin()) with check (public.is_admin());
...
create policy "banners_public_read_storage" on storage.objects
...
insert into public.products
  (legacy_id, slug, name, price, image, images, category, sold_out,
   position_top, position_left, mobile_position_top, mobile_position_left,
   rotation, scale, z_index)
values
  ('1',  'blue-y2-hoodie-1',          'Blue Y2 Hoodie',          49, '/products/1.png',  array['/products/1a.png','/products/1.png','/products/1b.png','/products/1c.png'],   'top', false, 'calc(22.5vh - 72px)', '2%',  'calc(15vh - 70px)', '5%',  -8,  0.90, 5),
  ('2',  'y2-t-shirt-no-1-2',         'Y2 T-Shirt No.1',         25, '/products/15.png', array['/products/15a.png','/products/15.png','/products/15b.png','/products/15c.png'], 'top', false, 'calc(33vh - 96px)',   '18%', 'calc(15vh - 70px)', '38%', 15,  0.85, 3),
  ('3',  'y2-t-shirt-no-13-3',        'Y2 T-Shirt No.13',        28, '/products/36.png', array['/products/36a.png','/products/36.png','/products/36b.png','/products/36c.png'], 'top', true,  'calc(18vh - 96px)',   '34%', 'calc(15vh - 70px)', '71%', 2,   0.95, 7),
  ('4',  'pink-y2-hoodie-4',          'Pink Y2 Hoodie',          49, '/products/2.png',  array['/products/2a.png','/products/2.png','/products/2b.png','/products/2c.png'],   'top', false, 'calc(28.5vh - 96px)', '50%', 'calc(37.5vh - 70px)','5%',  -3,  0.88, 4),
  ('5',  'y2-t-shirt-no-2-5',         'Y2 T-Shirt No.2',         25, '/products/23.png', array['/products/23a.jpg','/products/23.png','/products/23b.jpg','/products/23c.jpg'], 'top', false, 'calc(21vh - 96px)',   '66%', 'calc(37.5vh - 70px)','38%', -12, 0.98, 6),
  ('6',  'y2-t-shirt-no-14-6',        'Y2 T-Shirt No.14',        28, '/products/28.png', array['/products/28a.png','/products/28.png','/products/28b.png','/products/28c.png'], 'top', false, 'calc(21vh - 96px)',   '82%', 'calc(37.5vh - 70px)','71%', 8,   0.82, 2),
  ('7',  'y2-t-shirt-no-15-7',        'Y2 T-Shirt No.15',        28, '/products/27.png', array['/products/27a.png','/products/27.png','/products/27b.png','/products/27c.png'], 'top', false, 'calc(63vh - 96px)',   '8%',  'calc(60vh - 70px)', '5%',  45,  0.75, 8),
  ('8',  'y2-t-shirt-no-3-8',         'Y2 T-Shirt No.3',         25, '/products/25.png', array['/products/25a.png','/products/25.png','/products/25b.png','/products/25c.png'], 'top', false, 'calc(69vh - 96px)',   '24%', 'calc(60vh - 70px)', '38%', -5,  0.92, 3),
  ('9',  'brown-y2-hoodie-9',         'Brown Y2 Hoodie',         49, '/products/7.png',  array['/products/7a.png','/products/7.png','/products/7b.png','/products/7c.png'],   'top', false, 'calc(81vh - 96px)',   '40%', 'calc(60vh - 70px)', '71%', 12,  0.96, 5),
  ('10', 'y2-t-shirt-no-16-10',       'Y2 T-Shirt No.16',        28, '/products/29.png', array['/products/29a.png','/products/29.png','/products/29b.jpg','/products/29c.png'],'top', true,  'calc(72vh - 96px)',   '56%', 'calc(82.5vh - 70px)','5%',  -8,  0.84, 4),
  ('11', 'orange-y2-hoodie-11',       'Orange Y2 Hoodie',        49, '/products/8.png',  array['/products/8a.png','/products/8.png','/products/8b.jpg','/products/8c.png'],   'top', false, 'calc(72vh - 96px)',   '72%', 'calc(82.5vh - 70px)','38%', -35, 0.70, 9),
  ('13', 'red-y2-hoodie-13',         'Red Y2 Hoodie',           49, '/products/6.png',  array['/products/6a.png','/products/6.png','/products/6b.png','/products/6c.png'],   'top', false, 'calc(114vh - 96px)',  '4%',  'calc(105vh - 70px)','5%',  -10, 0.93, 4),
  ('14', 'y2-t-shirt-no-17-14',       'Y2 T-Shirt No.17',        28, '/products/32.png', array['/products/32a.png','/products/32.png','/products/32b.jpg','/products/32c.png'],'top', false, 'calc(120vh - 96px)',  '20%', 'calc(105vh - 70px)','38%', 25,  0.85, 7),
  ('15', 'y2-t-shirt-no-4-15',        'Y2 T-Shirt No.4',         25, '/products/19.png', array['/products/19a.png','/products/19.png','/products/19b.png','/products/19c.png'], 'top', false, 'calc(111vh - 96px)',  '36%', 'calc(105vh - 70px)','71%', 4,   0.91, 2),
  ('16', 'y2-t-shirt-no-18-16',       'Y2 T-Shirt No.18',        28, '/products/31.png', array['/products/31a.png','/products/31.png','/products/31b.jpg','/products/31c.png'],'top', false, 'calc(123vh - 96px)',  '52%', 'calc(127.5vh - 70px)','5%',-7,  0.98, 6),
  ('17', 'blue-y2-hoodie-17',         'Blue Y2 Hoodie',          49, '/products/3.png',  array['/products/3a.png','/products/3.png','/products/3b.png','/products/3c.png'],   'top', false, 'calc(117vh - 96px)',  '68%', 'calc(127.5vh - 70px)','38%',9,  0.86, 3),
  ('18', 'y2-t-shirt-no-5-18',        'Y2 T-Shirt No.5',         25, '/products/20.png', array['/products/20a.png','/products/20.png','/products/20b.png','/products/20c.png'], 'top', false, 'calc(117vh - 96px)',  '84%', 'calc(127.5vh - 70px)','71%',-2, 0.94, 5),
  ('19', 'y2-t-shirt-no-6-19',        'Y2 T-Shirt No.6',         25, '/products/17.png', array['/products/17a.png','/products/17.png','/products/17b.png','/products/17c.png'], 'top', false, 'calc(163.5vh - 96px)','2%',  'calc(150vh - 70px)','5%',  -6,  0.89, 5),
  ('20', 'y2-t-shirt-no-19-20',       'Y2 T-Shirt No.19',        28, '/products/30.png', array['/products/30a.png','/products/30.png','/products/30b.jpg','/products/30c.png'],'top', false, 'calc(169.5vh - 96px)','18%', 'calc(150vh - 70px)','38%', 11, 0.94, 4),
  ('21', 'green-y2-hoodie-21',        'Green Y2 Hoodie',         49, '/products/9.png',  array['/products/9a.png','/products/9.png','/products/9b.png','/products/9c.png'],   'top', false, 'calc(162.5vh - 96px)','34%', 'calc(150vh - 70px)','71%', -3,  0.86, 7),
  ('22', 'y2-t-shirt-no-20-22',       'Y2 T-Shirt No.20',        28, '/products/33.png', array['/products/33a.png','/products/33.png','/products/33b.jpg','/products/33c.png'],'top', false, 'calc(172.5vh - 96px)','50%', 'calc(172.5vh - 70px)','5%', -9, 0.91, 3),
  ('23', 'y2-t-shirt-no-7-23',        'Y2 T-Shirt No.7',         25, '/products/22.png', array['/products/22a.png','/products/22.png','/products/22b.png','/products/22c.png'], 'top', false, 'calc(165vh - 96px)',  '66%', 'calc(172.5vh - 70px)','38%',7,  0.84, 6),
  ('24', 'yellow-y2-hoodie-24',       'Yellow Y2 Hoodie',        49, '/products/12.png', array['/products/12a.png','/products/12.png','/products/12b.png','/products/12c.png'],'top', false, 'calc(166.5vh - 96px)','82%', 'calc(172.5vh - 70px)','71%',14, 0.80, 2),
  ('25', 'y2-t-shirt-no-21-25',       'Y2 T-Shirt No.21',        28, '/products/35.png', array['/products/35a.png','/products/35.png','/products/35b.png','/products/35c.png'], 'top', false, 'calc(210vh - 96px)',  '4%',  'calc(195vh - 70px)','5%', -14, 0.92, 4),
  ('26', 'light-pink-y2-hoodie-26',   'Light Pink Y2 Hoodie',    49, '/products/10.png', array['/products/10a.png','/products/10.png','/products/10b.png','/products/10c.png'],'top', false, 'calc(216vh - 96px)',  '20%', 'calc(195vh - 70px)','38%', 18, 0.87, 7),
  ('27', 'y2-t-shirt-no-8-27',        'Y2 T-Shirt N0.8',         25, '/products/16.png', array['/products/16a.png','/products/16.png','/products/16b.png','/products/16c.png'], 'top', false, 'calc(208vh - 96px)',  '36%', 'calc(195vh - 70px)','71%', -5, 0.82, 3),
  ('28', 'black-y2-hoodie-28',        'Black Y2 Hoodie',         49, '/products/11.png', array['/products/11a.png','/products/11.png','/products/11b.png','/products/11c.png'],'top', false, 'calc(220.5vh - 96px)','50%', 'calc(217.5vh - 70px)','5%', 5,  0.95, 6),
  ('29', 'y2-t-shirt-no-22-29',       'Y2 T-Shirt No.22',        28, '/products/34.png', array['/products/34a.png','/products/34.png','/products/34b.png','/products/34c.png'], 'top', false, 'calc(212.5vh - 96px)','66%', 'calc(217.5vh - 70px)','38%',-16,0.90, 5),
  ('30', 'y2-t-shirt-no-9-30',        'Y2 T-Shirt No.9',         25, '/products/24.png', array['/products/24a.png','/products/24.png','/products/24b.png','/products/24c.png'], 'top', false, 'calc(214vh - 96px)',  '82%', 'calc(217.5vh - 70px)','71%', 9,  0.78, 8),
  ('31', 'cream-y2-hoodie-31',        'Cream Y2 Hoodie',         49, '/products/14.png', array['/products/14a.png','/products/14.png','/products/14b.png','/products/14c.png'],'top', false, 'calc(253vh - 96px)',  '2%',  'calc(240vh - 70px)','5%', -8,  0.94, 4),
  ('32', 'y2-t-shirt-no-10-32',       'Y2 T-Shirt No.10',        25, '/products/26.png', array['/products/26a.png','/products/26.png','/products/26b.png','/products/26c.png'], 'top', false, 'calc(259vh - 96px)',  '18%', 'calc(240vh - 70px)','38%', 13, 0.86, 7),
  ('33', 'light-purple-y2-hoodie-33', 'Light Purple Y2 Hoodie',  49, '/products/13.png', array['/products/13a.png','/products/13.png','/products/13b.png','/products/13c.png'],'top', false, 'calc(252vh - 96px)',  '34%', 'calc(240vh - 70px)','71%', -11,0.90, 2),
  ('34', 'y2-t-shirt-no-23-34',       'Y2 T-Shirt No.23',        28, '/products/37.png', array['/products/37a.png','/products/37.png','/products/37b.png','/products/37c.png'], 'top', false, 'calc(263.5vh - 96px)','50%', 'calc(262.5vh - 70px)','5%', 6, 0.97, 6),
  ('35', 'y2-t-shirt-no-24-35',       'Y2 T-Shirt No.24',        28, '/products/38.png', array['/products/38a.png','/products/38.png','/products/38b.png','/products/38c.jpg'], 'top', false, 'calc(255.5vh - 96px)','66%', 'calc(262.5vh - 70px)','38%',-7, 0.88, 5),
  ('36', 'y2-t-shirt-no-11-36',       'Y2 T-Shirt No.11',        25, '/products/18.png', array['/products/18a.png','/products/18.png','/products/18b.png','/products/18c.png'], 'top', false, 'calc(257vh - 96px)',  '82%', 'calc(262.5vh - 70px)','71%', 16, 0.81, 3),
  ('37', 'gray-y2-hoodie-37',         'Gray Y2 Hoodie',          49, '/products/5.png',  array['/products/5a.png','/products/5.png','/products/5b.png','/products/5c.png'],   'top', false, 'calc(299.5vh - 96px)','6%',  'calc(285vh - 70px)','5%', -12, 0.93, 5),
  ('38', 'y2-t-shirt-no-25-38',       'Y2 T-Shirt No.25',        28, '/products/21.png', array['/products/21a.png','/products/21.png','/products/21b.png','/products/21c.png'], 'top', false, 'calc(305vh - 96px)',  '22%', 'calc(285vh - 70px)','38%', 20, 0.85, 7),
  ('39', 'purple-y2-hoodie-39',       'Purple Y2 Hoodie',        49, '/products/4.png',  array['/products/4a.png','/products/4.png','/products/4b.png','/products/4c.png'],   'top', false, 'calc(298vh - 96px)',  '52%', 'calc(285vh - 70px)','71%', -4,  0.89, 4)
on conflict (legacy_id) do update set
  slug = excluded.slug, name = excluded.name, price = excluded.price, image = excluded.image,
  images = excluded.images, category = excluded.category, sold_out = excluded.sold_out,
  position_top = excluded.position_top, position_left = excluded.position_left,
  mobile_position_top = excluded.mobile_position_top, mobile_position_left = excluded.mobile_position_left,
  rotation = excluded.rotation, scale = excluded.scale, z_index = excluded.z_index;

insert into public.product_inventory (product_id, size, available, reserved, sold)
select p.id, s.size, 10, 0, 0
from public.products p
cross join (values ('SMALL'), ('MEDIUM'), ('LARGE')) as s(size)
